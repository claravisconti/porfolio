...
export default function ProjectDetail() {
  ...
  // Scroll al inicio cada vez que cambia el proyecto
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [slug]);
  ...
  // Verificamos si el proyecto tiene contenido para la vista detallada
    ...
    <div className="bg-white min-h-screen text-black">
      ...
      {isDetailed ? (
        <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-16">
              {[
                ...
                <div key={i} className="flex flex-col items-center md:items-start text-center md:text-left gap-4 md:gap-6">
                  <div className="text-black/80" aria-hidden="true">{item.icon}</div>
              ...
              <div className={project.gallery?.[1] ? "md:col-span-4 space-y-12" : "flex flex-col md:flex-row justify-between items-center w-full"}>
                ...
                {project.fonts && (
                  <div className="flex flex-col items-center md:items-start">
                    ...
                    <p className="text-2xl md:text-3xl font-bold uppercase tracking-tighter">{project.fonts}</p>
                  </div>
                )}
              </div>
          ...
          {/* SECCIÓN 4: SOLUCIÓN Y MÉTRICAS (H3) */}
          <section className="max-w-7xl mx-auto px-6 md:px-16 py-12 md:py-12">
            <div className={`bg-[#fafafa] p-8 md:p-16 rounded-sm grid grid-cols-1 ${project.metrics?.length > 0 ? 'lg:grid-cols-2' : 'lg:grid-cols-1'} gap-16 items-start`}>
              ...
              {project.metrics?.length > 0 && (
                <div className="space-y-8 md:space-y-10" role="list">
                  {project.metrics.map((m, i) => (
                    <div key={i} className="group" role="listitem">
                      ...
                      <div className="h-[1px] bg-gray-200 relative" aria-hidden="true">
                        <div className="absolute h-full bg-black transition-all duration-1000" style={{ width: `${m.percentage}%` }} />
                      </div>
                    </div>
                  ))}
                </div>
        ...
        /* VISTA SIMPLE PARA CATEGORÍAS SIN DATOS ESTRATÉGICOS */
